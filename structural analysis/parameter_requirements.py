...
data_y_all_sec = cw.data_y_upper_all_sec
Mz = load_diagrams(100)[0]

def required_Izz(HalfspanValues, data_y_all_sec, y_centroid_all_sec, Mz):
    
    I_zz_req_all_sec = np.zeros((len(HalfspanValues), 1))
    sigma_ult = 380 * 10 ** 6
    
    for i in range(len(HalfspanValues)):
        y_max = max(data_y_all_sec[i]) - y_centroid_all_sec[i]
