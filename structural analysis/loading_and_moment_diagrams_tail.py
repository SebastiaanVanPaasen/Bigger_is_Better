...
S = 51.52#wing["S"]#286.02#184.16#193.72#220.27 # m^2
b = 14.36#wing["b"]#86.83#47.83#39.56#41.76#55.53
AR = 4.0#wing["A"]# 8.#8.5# 9 #14
taper = 0.3999#wing["Taper"]#0.31#0.4#0.31
Sweep0 = 0.64229#m.atan(m.tan(qcsweep) - 4 / AR * (-0.25 * (1 - taper) / (1 + taper))) # rad
qcsweep= m.atan(m.atan(sweep0)+(4./AR)*(-0.25*(1-taper)/(1+taper)))#wing["Sweep"]

#%%
Cr = 5.13#wing["C_root"] #8.54#7.11#6.63# 6.06(2 * S) / ((1 + taper) * b)  # (S + np.tan(Sweep0) * by * (b / 4)) / (by + (b - by) * ((1 + taper) / 2))
CD0 = 0.0177 
S = 51.52 
b = 14.36
AR = 4.0
...
Wing_W = 32965.94635#208030.9134#weights["W_W"]#212307.69#149254.55#161567.21# 285629.97
