...
from class_I_empennage_landinggear import class_I_empennage
...
while i < maximum and percentage > 0.01:
    ...
    QC_sweep, LE_sweep, b, c_root, c_tip, dihedral, t_over_c, mac, taper = wing_parameters(M_cruise, CL_cruise, S, A,
                                                                                           wing_option)
    ...
    cg_locations, tail_h, tail_v, x_lemac, avl_h, avl_v = class_I_empennage(mass_fractions, mac, l_fuselage, x_engines,
                                                              l_nacelle, xcg_oew_mac, x_payload, x_fuel,
                                                              d_fuselage, b, S, taper, v_tail, LE_sweep, h_tail)
    ...
    make_avl_file(c_root, c_tip, b, LE_sweep, dihedral, S, CD_0, M_cruise, avl_h, b_h, c_root_h, QC_sweep_h, tap_h, avl_v,
    ...
    prop_sys_weight = (
                              w_engine * N_engines) / lbs_to_kg + ai_weight + prop_weight + fuel_sys_weight + w_ec + w_ess + w_pc + w_osc
    ...
    empty_weight = np.append(empty_weight, W_E_II)
