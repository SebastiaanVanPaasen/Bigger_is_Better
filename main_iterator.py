...
# Number of props, blades per prop, prop diameter and prop efficiency
prop_characteristics = [2, 50, 1.5, 0.8]
...
QC_sweep, LE_sweep, taper_ratio, b, root_chord, tip_chord, dihedral, t_over_c, mac = wing_parameters(M_cruise,
                                                                                                     CL_cruise, S, A)
...
w_weight = wing_weight(weights[0], weights[3], b, HC_sweep, n_ult, S, t_max_root, [0, 0, 0, 0, 0, 0])
emp_weight = empennage_weight([0, 0], [S_h, S_v], V_D, [HC_sweep_h, HC_sweep_v], z_h, b_v)

# Note that the fuselage is assumed circular in this case
fus_weight = fuselage_weight(0, V_D, l_h, d_fuselage, d_fuselage, S_fus_gross)

# Note that the choice includes the engine choice here
nac_weight = nacelle_weight(weights[0], 0)
lg_weight = landing_gear_weight(weights[0])
eng_weight = engine_weight(N_engines, w_engine)

w_structural = w_weight + emp_weight + fus_weight + nac_weight + lg_weight + eng_weight
...
# Note that choice is regarding type of
fuel_sys_weight = fuel_system_weight(N_engines, n_fuel_tanks, weights[3], 0)
...
prop_sys_weight = propulsion_system_weight([0, 0, 0], l_fuselage, b, N_engines, w_engine, n_blades, n_prop, d_prop,
                                           to_power)

q_d = 0.5*Rho_Cruise*V_D
fix_eq_weight = fixed_equipment_weight(q_d, weights[0], weights[1], weights[3], V_pax, N_crew, N_pas, S_ff, R_max, 0)

# q_d, w_to, w_e, w_f, v_pax, n_crew, n_pax, s_ff, r, choice