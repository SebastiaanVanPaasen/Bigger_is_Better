# -*- coding: utf-8 -*-
"""
Created on Tue Apr 30 09:11:40 2019
...
def fuselage_cross_section(Npax,Npax_below):
    ...
    if Npax_below==Npax:
        ...
        if Nsa<=6:
            ...
            N_left = Nsa-N_right
            N_middle = 0
        elif Nsa>=6 and Nsa<=12:
            kcabin = 1.17
            Naisles = 2
            if Nsa < 9:
                N_left = 2
                N_right = 2
            elif Nsa >=9 and Nsa <= 12:
                N_left = 3
                N_right = 3
            N_middle = Nsa-N_left-N_right
        ...
        W_cabin = Nsa*Wseat + (Nsa+Naisles+1)*Warmrest+Naisles*Waisle+2*Sclearance
        ...
        r = make_circle([cabin_left_point,cabin_right_point,headroom_left_point,headroom_right_point,aisle_left_point,aisle_right_point])
        d_inner = 2*r[2]
...
#        plt.show()
        ...
        lcabin_above = 0
        N_aisle_above = 0
        N_aisle_below = Naisles
    #Same applies for a double decker configuration    
    elif Npax_below!=Npax:
        tot_seating_abreast = np.zeros([2,3])
        Npax_above = 450- Npax_below
        Nsa_below = round(0.45*sqrt(Npax_below))
        Nsa_above = round(0.45*sqrt(Npax_above))
        Nsa = [Nsa_below,Nsa_above]
        for i in range(len(Nsa)):    
            if Nsa[i]<=6:
                kcabin = 1.08
                Naisles = 1
                N_right = Nsa[i]/2
                N_left = Nsa[i]-N_right
                N_middle = 0
            elif Nsa[i]>=6 and Nsa[i]<=12:
                kcabin = 1.17
                Naisles = 2
                if Nsa[i] < 9:
                    N_left = 2
                    N_right = 2
                elif Nsa[i] >=9 and Nsa[i] <= 12:
                    N_left = 3
                    N_right = 3
                N_middle = Nsa[i]-N_left-N_right
            else:
                print("This aircraft have more than 2 aisle")
            if i == 0:
                tot_seating_abreast[1,0]=N_right
        ...
        lcabin_below = (Npax_above/Nsa_above)*kcabin
        lcabin_above = (Npax_below/Nsa_below)*kcabin
        lcabin = max(lcabin_below,lcabin_above)
        W_cabin_above = Nsa_above*Wseat + (Nsa_above+N_aisle_above+1)*Warmrest+N_aisle_above*Waisle+2*Sclearance
        W_cabin_below = Nsa_below*Wseat + (Nsa_below+N_aisle_below+1)*Warmrest+N_aisle_below*Waisle+2*Sclearance
        ...
        #cabin points below
        W_cabin_left_below_x = -W_cabin_above/2
        W_headroom_left_below_x = -W_headroom_above/2
        W_cabin_right_below_x = W_cabin_above/2
        W_headroom_right_below_x = W_headroom_above/2
        ...
        r = make_circle([cabin_left_point_above,cabin_left_point_below,cabin_right_point_below,cabin_right_point_above,headroom_left_point_below,headroom_left_point_above,headroom_right_point_below,headroom_right_point_above,aisle_left_point_below,aisle_left_point_above,aisle_right_point_below,aisle_right_point_above,shoulder_left_point_below,shoulder_right_point_below,shoulder_left_point_above,shoulder_right_point_above])
        d_inner = 2*r[2]
        d_outer = 1.045*d_inner+0.084
    ...
    l_tailcone_ratio=(2.6*d_outer,4*d_outer)
    l_nosecone_ratio=(1.2*d_outer,2.5*d_outer)
    ...
    #Length tailcone ratio  [5]
    #Length nosecone ratio  [6]
    #length fuselage        [7]
    #tot seating abreast    [8]
    #Number of aisles above [9]
    #Number of aisles below [10]
    return (d_inner,d_outer,lcabin,lcabin_below,lcabin_above,l_tailcone_ratio,l_nosecone_ratio,l_fuselage,tot_seating_abreast,N_aisle_above,N_aisle_below)
