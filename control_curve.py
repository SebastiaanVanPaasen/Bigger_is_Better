...
def dflap_Cm_ac(delta_flap,b,b_f0,b_fi,taper,c,c_f,dc_c_f,mu_2,mu_3,A,qcsweep,x_ac,CL_land):
    ...
    theta_f = np.cos(2*(c_f/c) - 1.)**(-1.)                                     
    mu_1 = 0.5*(1.-(c_f/c))*((np.sin(theta_f))/(np.pi -(theta_f-np.sin(theta_f))))     #angle on flap geometry depending on chord
                                                                                #correction factor for camber effect 3D wing
...
"""Coefficients"""
Cm_0       =   -0.1 #airfoil dependent moment coefficient [1/rad]
CL_0       =   0.4 #CL of flapped wing at angle of attack of 0 deg [1/rad]
CL_land    =   1.3 #Cl during landing full flap configuration [1/rad]
...
"""Retrieve from stability cruve program"""
x_ac       =  0.25 # aerodynamic centre location over the mac [n/m]
Vh_V      =  0.8 #(V_h/V) ratio  [m/s / m/s]
CL_alpha_AH = 4.788 # lift gradient of tailless aircraft [1/rad]


#------------------------------VALUES GRAPHS--------------------------
mu_2       =  1. #2D to 3D correction factor from graph  
mu_3       = 0.047#  correction factor for sweep from graph
dc_c_f     =  0.4 # flap geometry ratio, see torenbeek book

x_cg = np.linspace(0.,1.1,100)
