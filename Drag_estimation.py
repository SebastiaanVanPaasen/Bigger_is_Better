...
def Wing_wetted_area(root_chord, tip_chord, D_fuselage, span, S, winglet_height): #set winglet area to zero if not needed, select correct input for boxed wing
    ...
    exposed_area = S-(0.5*(fuselage_chord+root_chord))*D_fuselage + winglet_area  #calulate the exposed area, substracting the area inside the plane from the S
...
#using an adsee relation the fuselage wetted area is calulated
...
def Zero_Lift_Drag_est(S, Wing_wetted_area, H_wetted_area, V_wetted_area, Fus_wetted_area):
    CD_0 = 1.15*(1/S)*(0.003*Wing_wetted_area+ 0.0024*Fus_wetted_area + 0.0025*(H_wetted_area+V_wetted_area)) #adsee relations add 15% instead of 10 becuase nacelles are neglected
    return CD_0



Wing_wetted_area = Wing_wetted_area(root_chord, tip_chord, D_fuselage, span, S, winglet_height) #change function and add winglet height for the boxed wing
